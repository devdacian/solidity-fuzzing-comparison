...
//
// Medusa & Echidna Fuzzer info:
//
// Medusa is working best, recommend to use it by compiling from
// source @ https://github.com/crytic/medusa/ since the current
// official release is missing some fixes.
//
// configure solc-select to use compiler version:
// solc-select use 0.8.23 
//
// run from base project directory with:
// medusa --config test/08-omni-protocol/OmniAdvancedMedusa.json fuzz
// echidna --config test/08-omni-protocol/OmniAdvancedEchidna.yaml ./ --contract OmniAdvancedMedusa
//
// view html coverage reports:
// test/08-omni-protocol/coverage-medusa-advanced/coverage_report.html
// test/08-omni-protocol/coverage-echidna-advanced/covered.X.html (biggest X = latest run)
//
// in the reports search for OmniPool & OmniToken
// Medusa coverage: OmniPool 78% OmniToken 82% including successful liquidations
// Echidna coverage: not as good, liquidation isn't working
// 
// Using Medusa most of the important user functionality inc liquidation gets executed
//
// 2/16 invariants can be broken; Medusa typically breaks both in 1 run
// within 2 minutes and often much faster, while Echidna sometimes breaks 1 invariant
// within 5 minutes. Foundry is unable to break any invariants within 5 minutes.
//
interface IHevm {
    function prank(address) external;
}

contract OmniAdvancedMedusa {
    IHevm hevm = IHevm(address(0x7109709ECfa91a80626fF3989D68f67F5b1DD12D));

    using SubAccount for address;

    // make these constant to match Echidna & Medusa configs
    ...
    function medusa_tranche_borrow_deposit_shares_integrity() public view returns(bool) {
        return _getTranchBorrowDepositShareIntegrity(address(oToken),  0) &&
    ...
    function medusa_subaccount_one_isolated_market() public view returns(bool) {
        return !_inMoreThanOneIsolatedMarket(ALICE) &&
    ...
    function medusa_subaccount_one_mode() public view returns(bool) {
        return !_inMoreThanOneMode(ALICE) &&
    ...
    function medusa_cant_enter_isolated_market_with_active_borrows() public view returns(bool) {
        return !_hasEnteredIsolatedMarketWithActiveBorrows(ALICE) &&
    ...
    function medusa_cant_exit_market_or_mode_with_active_borrows() public view returns(bool) {
        return !_hasExitedMarketOrModeWithActiveBorrows(ALICE) &&
    ...
    function medusa_cant_enter_mode_with_entered_markets() public view returns(bool) {
        return !_hasEnteredModeWithEnteredMarkets(ALICE) &&
    ...
    function medusa_cant_enter_expired_market_or_mode() public view returns(bool) {
        return !_hasEnteredExpiredMarketOrMode(ALICE) &&
    ...
    function medusa_cant_borrow_without_entering_market_or_mode() public view returns(bool) {
        return !_hasLoanWithoutEnteringMarketOrMode(ALICE) &&
               !_hasLoanWithoutEnteringMarketOrMode(BOB);
    }

    
    // INVARIANT 9) subaccount should receive shares when making a deposit
    // note: this invariant is currently failing during some runs when the fuzzer
    // does small deposits due to a rounding-down-to-zero precision loss at
    ...
    function medusa_deposit_receives_shares() public view returns(bool) {
        return !_hasDepositWhichReceivedZeroShares(ALICE) &&
    ...
    function medusa_deposit_receives_correct_amount() public view returns(bool) {
        return !_hasDepositWhichReceivedIncorrectAmount(ALICE) &&
    ...
    function medusa_withdraw_decreases_shares() public view returns(bool) {
        return !_hasWithdrawWhichDecreasedZeroShares(ALICE) &&
    ...
    function medusa_withdraw_receives_correct_amount() public view returns(bool) {
        return !_hasWithdrawWhichReceivedIncorrectAmount(ALICE) &&
    ...
    // note: this invariant is currently failing during some runs when the fuzzer
    // does small repayments due to a rounding-down-to-zero precision loss at
    ...
    function medusa_repay_decreases_borrow_shares() public view returns(bool) {
        return !_hasRepayWhichDidntDecreaseBorrowShares(ALICE) &&
    ...
    function medusa_repay_correctly_decreases_borrow_amount() public view returns(bool) {
        return !_hasRepayWhichIncorrectlyDecreasedBorrowAmount(ALICE) &&
    ...
    function medusa_borrow_increases_borrow_shares() public view returns(bool) {
        return !_hasBorrowWhichDidntIncreaseBorrowShares(ALICE) &&
    ...
    function medusa_borrow_correctly_increases_borrow_amount() public view returns(bool) {
        return !_hasRepayWhichIncorrectlyDecreasedBorrowAmount(ALICE) &&
               !_hasRepayWhichIncorrectlyDecreasedBorrowAmount(BOB);
    }
